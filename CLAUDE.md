...
1. **`crates/adapters/src/registry/<name>.rs`** (or `<name>/client.rs` — see below) — implement `RegistryClient` (`registry_type`, `resolve_metadata`, `fetch_artifact`; optionally `list_versions`, `search_packages`).
   ...
   - **`PackageMetadata::checksum` must be bare hex or an SRI `<algo>-<base64>` token** — `integrity::parse_expected` reads nothing else, and a `"sha256:<hex>"` value silently disables cache-write verification with one `WARN` per download as its only symptom (RFC 0031 §13).
...
3. **`crates/web/src/handlers/proxy/<name>.rs`** — actix-web handlers; use `proxy_stream`, `require_registry_type`, `require_local_mode`, `content_type_for` helpers from `common.rs`. **Validate any package name/version taken from the request** with `batlehub_core::services::validate_package_name` (and reject `..`/separators in version) before it reaches a storage key — see the PyPI/Composer handlers. Two funnels enforce this in depth — `ProxyService::handle` (every proxy read) and `LocalRegistryService::get_artifact` (every local read) call `validate_coordinate` on the `PackageId`, and `ensure_safe_key` guards the storage backends as the last line of defense — but a handler that builds a storage key directly (e.g. the Maven, NuGet flat, and Terraform-provider paths) must still validate at the edge for a clean `400`, not rely on the deeper guards.
   **If the client sends its credential as anything but `Bearer` or `Basic`, normalise it in `crates/web/src/extractors.rs::raw_auth_from_request`**, scoped by the registry's *type* and not by the path. No `AuthProvider` reads any other shape, so the failure is silent and inverted: an authenticated read arrives anonymous and a closed registry refuses the client holding its token. Four kinds need this already — NuGet's `X-NuGet-ApiKey`, cargo's scheme-less bare token, `ovsx`'s `?token=` query param, galaxy's `Token` scheme. Read the client's source for the constant rather than assuming; RFC 0031 quoted the wrong one of two token classes and nothing below a real client could tell (§13).
...
5. **`crates/core/src/entities/registry_kind.rs`** — add the new variant to the `RegistryKind` enum and its `ALL` slice; `as_str`/`FromStr` and `crates/config/src/schema/mod.rs`'s `validate()` (which just calls `registry_type.parse::<RegistryKind>()`) pick it up automatically. `server/src/builders.rs`'s exhaustive match on `RegistryKind` will then force you to wire up client construction for the new variant. **Three tiers of per-kind dispatch, and only the first is free**: the compiler forces `as_str`, `listing_filter`, `readme_support`, `upstream_detail`, `fetchable_by_version`, `blocking::strip`, `upstream_detail::listing_carries_{readmes,links}`, `handlers/security.rs`'s `native_body` and both `builders.rs` matches; the five gates of step 9 are green-build-red-test; and **one function is silent** — `services/listing_synthesis.rs`'s `render_listing` matches `(RegistryKind, DocumentKind)` tuples and ends `_ => return None`, so a kind that omits it has an air gap that refuses every listing with no compile error and no failing test. See `docs/contributing/adding-a-registry.md` §6.
...
9. **Prove it with a real client, both ways** — a kind is not finished until it has been driven *live* and *air-gapped*, because every registry defect this project has shipped was found by a client and not by a test double (the ovsx download URL, the GitLab release document, the JetBrains numeric-id spelling):
   ...
   **Five gates fail the build for a kind that is missing one**, and none of them is a compile error — you meet them one red run at a time, after the code builds: `authz.sh`'s `authz_check_kinds_covered`, `registry_kind_coverage.rs` (the live phase *and* the air-gap claim, both checked against the files), `soak_kind_coverage.rs`, `authz_matrix.rs` (two route inventories, which also catch a *rename*), and the README-extractor check in `sbom/extractor/mod.rs`. The table naming what each one demands is `docs/contributing/adding-a-registry.md` §11; walk it deliberately rather than being told.

   **Write the suite, then run it, then believe the kind works — in that order.** `apk` shipped with 43 green tests and five defects a real `apk` found in an hour (RFC 0026 §13); `galaxy` reached 5 478 green tests across 149 suites and still had five defects a real `ansible-galaxy` found in twenty minutes (RFC 0031 §13). A test suite cannot disagree with the document it was written from.

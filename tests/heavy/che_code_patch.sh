...
che_code_version() {  # <tree>
  local tree="$1"
  python3 -c 'import json,sys;print(json.load(open(sys.argv[1]))["version"])' "$tree/product.json"
  return $?
}
...
if [[ -n "${CHE_CODE_DIR:-}" ]]; then
  ...
  if [[ ! -f "$CHE_DIR/product.json" ]]; then
    ...
    cp -r "$POD_TREE" "$CHE_DIR.partial" || heavy_fail "could not copy $POD_TREE into $CHE_DIR.partial"

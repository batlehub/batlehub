...
pub async fn import_bundle(
    ...
    let (trusted, air_gapped, registries, verdicts, refs) = {
        let hot = svc.hot.read().await;
        (
    ...
    for entry in &read.manifest.entries {
        ...
        // this it would be a way to plant content at any key.
        if !registries.iter().any(|r| r == &entry.registry) {
        ...
        let Some(bytes) = read.blobs.get(&entry.digest) else {
            rejections.push(format!(
        ...
        // The coordinate this entry names, worked out once: the metadata
        // row, the cache entry and the verdict all file under it.
        ...
        if let Err(e) = svc
            ...
            .await
        {
            // The bytes are in; the row that lets the cache find them is
            // not. Reported rather than swallowed: the artifact would be
            // served but never expired or evicted.
            rejections.push(format!(
                "{}: stored, but its metadata row failed: {e}",
                entry.key
            ));
        }
        // The metadata entry that lets the read path find those bytes.
        //
        // Without it an imported artifact is unreachable: `ProxyService`
        // resolves metadata *before* it looks at the cache, and on an
        // air-gapped instance the resolve has no upstream to ask — so a
        // bundle would import cleanly, report its blobs written, and serve
        // `503` for every one of them. The two keys are the same coordinate
        // under two prefixes (`artifact:` and `meta:`), which is why one
        // reported key is enough to write both.
        //
        ...
        // judgement that *did* have the date is the verdict below, made on
        // the connected side and carried across.
        if let Err(e) = svc
            .cache
            .set(
                &format!("meta:{}", entry.key),
                batlehub_core::ports::CacheEntry {
                    ...
                    // to refresh from, and an entry that expired would take
                    // the artifact it finds with it. On a *connected* one —
                    // the staging instance that builds bundles, §4.5's second
                    // warning — it expires at once, so the import seeds bytes
                    // without pinning a metadata answer the upstream would
                    // have given better.
                    expires_at: (!air_gapped).then(Utc::now),
                },
                None,
            )
            .await
        ...
        // RFC 0008 §13.3: the ref → commit pair, so the disconnected
        // instance can answer `tarball/main` without resolving anything.
        //
        // This is not a nicety on a forge registry: a ref is resolved
        // *before* anything is fetched, and an instance with no forge to ask
        // would refuse every coordinate in the bundle it just accepted. The
        // TTL is frozen under `air_gap.enabled` for the same reason, so a row
        // written here is never re-asked.
        if let (Some(store), Some(r)) = (&refs, &entry.git_ref) {
            // A kind this instance does not understand is refused rather
            // than defaulted. Guessing `tag` for a branch would lose the
            // `MUTABLE_REF` finding that makes following a branch visible —
            // a silent downgrade of a warning, which is the worst way to be
            // wrong about a ref.
            match r.kind.parse::<batlehub_core::entities::RefKind>() {
                Ok(kind) => {
                    let resolution = batlehub_core::ports::StoredRefResolution {
                        kind,
                        sha: r.sha.clone(),
                        resolved_at: entry.verified_at.unwrap_or(read.manifest.created_at),
                        previous: None,
                    };
                    if let Err(e) = store
                ...
                Err(_) => rejections.push(format!(
                    "{}: stored, but its ref names a kind this instance does not know ('{}')",
                    entry.key, r.kind
                )),
            }
        }
        // RFC 0008 §13 decision 4: the judgement crosses the gap with the
        // bytes. A disconnected instance cannot re-run a scanner, so an
        // imported artifact with no verdict is `SCAN_PENDING` on a registry
        // with `[security]` — fail-closed, and the bundle it just accepted
        // would serve nothing. The row says where the judgement came from:
        // `policy_ref` names the bundle, never the local policy, so nothing
        // here can read as a check this instance made.
        if let (Some(store), Some(state)) = (&verdicts, carried_verdict(entry)) {
            let verdict = batlehub_core::entities::Verdict {
                ...
                reason_codes: entry
                    ...
                    .collect(),
                findings: vec![],
                policy_ref: format!("bundle:{}", read.manifest.bundle_id),
                available_at: None,
                evaluated_at: entry.verified_at.unwrap_or(read.manifest.created_at),

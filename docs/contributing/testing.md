...
| **External integration** | `crates/adapters/tests/*.rs` | real Postgres / RustFS(S3) / Redis via Podman | `task test:pg-*`, `task test:s3` |
...
task test:s3                  # RustFS   — S3StorageBackend (feature storage-s3)
...
# Coverage (starts Postgres + RustFS + Redis; HTML report in coverage/html/)
task coverage
...
| `s3_storage.rs` | RustFS / S3 (`storage-s3`) | `S3StorageBackend` | `test:s3` |

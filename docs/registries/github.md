...
repos          = ["cli/*"]         # owner/repo globs; empty allows any repository
...
- Attestations and commit signatures are read where GitHub offers them: a release asset's build attestation (`/attestations/{digest}`, github.com and GitHub Enterprise 3.13 or later) and the commit's own `verification` object. What they say reaches the verdict as `PROVENANCE_MISSING` or `PROVENANCE_INVALID`; a forge with nothing to say reports missing, never a guess.

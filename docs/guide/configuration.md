...
| Field | Type | Required | Notes |
|---|---|---|---|
| `type` | string | yes | `"github"`, `"forgejo"`, `"gitlab"`, `"npm"`, `"cargo"`, `"nuget"`, `"openvsx"`, `"vscode-marketplace"`, `"goproxy"`, `"maven"`, `"terraform"`, `"rubygems"`, `"composer"`, `"pypi"`, `"conda"`, `"deb"`, `"rpm"`, `"pacman"`, `"jetbrains"`, `"jetbrains-marketplace"`, `"generic"`, `"nodedist"`, `"sdkman"`, `"rustup"`, `"apk"`, `"galaxy"`, `"nix"` |
| `name` | string | yes | Unique identifier; used in proxy URL paths |
| `mode` | string | no | `"proxy"` (default), `"local"`, or `"hybrid"`. Supported for `cargo`, `npm`, `nuget`, `openvsx`, `vscode-marketplace`, `jetbrains-marketplace`, `goproxy`, `maven`, `terraform`, `rubygems`, `composer`, `pypi`, `conda`, `deb`, `rpm`, `pacman`, `apk`, `galaxy`, and `nix`. See [registry modes](#registry-modes). |
...
| `deny_components` | string[] | no | **rustup only.** Manifest package names (`rust-docs`, `clippy-preview`) this registry never serves, whatever the channel manifest says — applied before `[renames]`, because that is the table the filter edits. A profile install proceeds without them; an explicit `rustup component add` stops on rustup's own *"toolchain '…' does not contain component '…'"*. Empty denies nothing. Rejected on any other type ([RFC 0024](/rfc/0024-rustup-dist) §4.5). See [Rust toolchain](/registries/rustup). |
| `roles` | string | no | **galaxy only.** How much of Ansible Galaxy's **v1 role** surface this registry serves: `"proxy"` (default — reads served, `download_url` rewritten here, the archive fetched server-side through the SSRF guard), `"index"` (reads served, `download_url` relayed, no role-byte egress), or `"off"` (the v1 endpoints `404` **and** `v1` is absent from the discovery document, so `ansible-galaxy role install` fails on its own *"requires API versions 'v1'"*). Rejected on any other type ([RFC 0031](/rfc/0031-ansible-galaxy) §4.4). See [Ansible Galaxy](/registries/galaxy#the-roles-setting). |
| `require_upstream_sigs` | bool | no | **nix only.** `false`. Refuse to relay a narinfo that carries no `Sig:` line at all. Off by default because a content-addressed path (`CA:`) legitimately has none and the client's own `require-sigs` is the check that protects the store; on, it closes the one case the client cannot see — an upstream mirror that silently dropped signatures ([RFC 0028](/rfc/0028-nix-binary-cache) §4.1). |
| `pending_nar_ttl_secs` | int | no | **nix only.** `3600`. How long an unclaimed NAR upload waits in staging before a later upload sweeps it. `nix copy --to` sends the NAR before the narinfo that names it, so the bytes have no coordinate of their own in between. Raise it for a link where the two requests are far apart — not for a large closure, which `nix copy` walks path by path ([RFC 0028](/rfc/0028-nix-binary-cache) §4.4). |
| `max_pending_nars` | int | no | **nix only.** `64`. How many unclaimed NAR uploads one publisher may hold at once; the next answers `429`. The floor that matters is `nix copy`'s own concurrency (`http-connections`, default 25) — below that it refuses legitimate copies. |
| `apk_unsigned` | bool | no | **apk only.** `false`. Ship a local/hybrid `apk` repository whose index is unsigned. Every stock `apk` refuses it without `--allow-untrusted`, which also switches off the package identity check — so this is a choice put on the record, never inherited ([RFC 0026](/rfc/0026-alpine-apk) §4.1). |
| `path_allow` | string[] | no | Glob allowlist of upstream paths this registry may serve. Only valid for the path-addressed types (`deb`, `rpm`, `pacman`, `apk`, `jetbrains`, `generic`) — using it elsewhere is a config error. **Required and non-empty for `generic`.** Use `["**"]` to allow everything deliberately. |
...
| `apk_signing` | table | no | **apk only.** The RSA key this registry signs its generated `APKINDEX.tar.gz` with — apk-tools verifies RSA, so this one is not the Ed25519 signer `deb`/`rpm` use — plus any `previous_keys` still trusted by deployed clients. `local`/`hybrid` only: in proxy mode the upstream index is relayed byte-exact and there is nothing to sign. An air-gapped instance needs it even in `proxy` mode to answer `apk update` at all. See [Alpine / apk](/registries/apk). |
| `nix_signing` | table | no | **nix only.** The Ed25519 key this registry signs the narinfos it *hosts* with, so a client running the stock `require-sigs = true` accepts a locally published store path. `local`/`hybrid` only — a proxied narinfo keeps the upstream's `Sig:` lines byte-exact and is never re-signed. See [Nix binary cache](/registries/nix). |
...
`cargo`, `npm`, `nuget`, `openvsx`, `vscode-marketplace`, `jetbrains-marketplace`, `goproxy`, `maven`, `terraform`, `rubygems`, `composer`, `pypi`, `conda`, `deb`, `rpm`, `pacman`, `apk`, `galaxy`, and `nix` registries support three operating modes, set via the `mode` field. The rest — the git forges, `jetbrains`, `generic`, `nodedist`, `sdkman` and `rustup` — are proxy-only, because they have no publish protocol to host: Node releases are built by the Node project, SDKMAN's candidates by their vendors and Rust's toolchains by the release team.
...
> - **Alpine (`apk`)** — every package an `APKINDEX` lists carries its build time in `t:` (measured: 5 647 of 5 647 in v3.22/main/x86_64), so the undated case is narrow and specific — a coordinate the *cached* index no longer lists. **Mandatory** here too: `true` refuses it, `false` serves it (RFC 0026 §4.5).
> - **Ansible Galaxy (`galaxy`)** — every upstream collection version carries `created_at`, so the field is inert in proxy mode. The undated case is a locally published collection or an air-gapped listing, which is exactly where the two postures diverge, so it is **mandatory**: `true` refuses it, `false` serves it (RFC 0031 §4.5).
> - **Nix binary cache (`nix`)** — a narinfo carries hashes, a closure and a deriver, and no date anywhere in the protocol. Every store path therefore reaches the gate undated and the field is not a tie-break but the whole rule, so it is **mandatory**: `true` refuses every substitution on the registry, `false` makes the gate inert (RFC 0028 §4.5).
> - **Rust toolchains (`rustup`)** — a toolchain takes its date from the dated `dist/` directory its files live under (`manifests.txt` recovers it for a stable release), so the gate is enforced normally. The one exception is the `rustup` **installer** package itself: its tree publishes no dates, so `rustup-init` reaches the gate undated and `deny_missing_timestamp` decides it. Not mandatory here — it keeps its usual `false` default.
...
> This checks `PackageMetadata.is_signed`, a best-effort signal populated per registry adapter — not full cryptographic signature verification. GitHub, Forgejo, GitLab, OpenVSX, VS Code Marketplace and **nix** populate it (presence of a `.asc`/`.sig` release asset, an extension signature blob, or — for `nix` — at least one `Sig:` line on the narinfo); registries with no signing concept in their ecosystem (npm, PyPI, crates.io, Maven, RubyGems, Conda, Composer, Go, Terraform, NuGet, deb/rpm/pacman/apk, Ansible Galaxy, rustup) report `None` and are allowed through unless `deny_missing_signature = true`.
>
...
`pacman`, `apk`, `jetbrains`, `generic`), whose rows and blocks then name the file's
...
(`air-gap.listing-not-synthesised`). `apk` is the one OS-package kind absent
from that list: it holds its own signing key, so a disconnected instance
composes and signs an `APKINDEX.tar.gz` over the packages it actually has —
see [Alpine / apk](/registries/apk).

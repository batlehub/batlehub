...
export const sidebar = {
  ...
  "/rfc/": [
    ...
    // The shelves are sidebar *sections*, one level deep like every other space
    // here, rather than groups nested inside "Design history". A nested group is
    // the one shape VitePress renders as a `role="button"` heading wrapping a
    // focusable caret — `axe:nested-interactive` on all 126 RFC pages — with the
    // title as an `h3` at the sidebar's 14px, which is off the type ramp. Two
    // rendered-gate failures for a fold nobody asked for.
    // BEGIN rfc-sidebar — generated by `task rfc:index` from each RFC's header table — do not edit by hand
    {
      ...
      items: [
        ...
        {
          text: "0033 — Authenticated JetBrains access",
          link: "/rfc/0033-authenticated-jetbrains-marketplace-access",
        },
        {
